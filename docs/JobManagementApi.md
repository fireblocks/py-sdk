...
with fireblocks_client.ApiClient(configuration) as api_client:
    ...
    api_instance = fireblocks_client.JobManagementApi(api_client)
    job_id = 'job_id_example' # str | The requested job id
    idempotency_key = 'idempotency_key_example' # str | A unique identifier for the request. If the request is sent multiple times with the same idempotency key, the server will return the same response as the first request. The idempotency key is valid for 24 hours. (optional)

    try:
        # Cancel a running job
        api_instance.cancel_job(job_id, idempotency_key=idempotency_key)
...
with fireblocks_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Continue a paused job
        api_instance.continue_job(job_id, idempotency_key=idempotency_key)
    except Exception as e:
...
with fireblocks_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Return a list of tasks for given job
        api_response = api_instance.get_job_tasks(job_id)
...
with fireblocks_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Return a list of jobs belonging to tenant
        api_response = api_instance.get_jobs(from_time=from_time, to_time=to_time)
...
with fireblocks_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Pause a job
        api_instance.pause_job(job_id, idempotency_key=idempotency_key)
    except Exception as e:
