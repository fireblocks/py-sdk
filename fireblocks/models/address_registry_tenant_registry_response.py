...
class AddressRegistryTenantRegistryResponse(BaseModel):
    """
    Workspace participation in the address registry. Same shape for GET, POST (opt in), and DELETE (opt out).
    """ # noqa: E501
    status: StrictStr = Field(description="OPTED_IN or OPTED_OUT.")
    __properties: ClassVar[List[str]] = ["status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OPTED_IN', 'OPTED_OUT']):
            raise ValueError("must be one of enum values ('OPTED_IN', 'OPTED_OUT')")
        return value
