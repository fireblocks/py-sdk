...
class InstaPayPaymentInfo(BaseModel):
    """
    InstaPay payment information for Philippine instant payment transfers (PHP)
    """ # noqa: E501
    rail: StrictStr = Field(description="The payment rail type for InstaPay transfers")
    addressing_system: StrictStr = Field(description="The addressing system used for InstaPay transfers", alias="addressingSystem")
    ...
    account_number: StrictStr = Field(description="Recipient bank account or wallet number", alias="accountNumber")
    bank_name: StrictStr = Field(description="Name of the recipient's bank or wallet (e.g. BDO, BPI, GCash, Maya)", alias="bankName")
    __properties: ClassVar[List[str]] = ["rail", "addressingSystem", "accountHolderGivenName", "accountHolderSurname", "accountNumber", "bankName"]

    @field_validator('rail')
    def rail_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['INSTA_PAY']):
            raise ValueError("must be one of enum values ('INSTA_PAY')")
        return value
