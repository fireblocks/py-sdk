...
class Settlement(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = Settlement.model_construct()
        error_messages = []
        match = 0
        # validate data type: PrefundedSettlement
        if not isinstance(v, PrefundedSettlement):
            error_messages.append(f"Error! Input type `{type(v)}` is not `PrefundedSettlement`")
        else:
            match += 1
        # validate data type: DVPSettlement
        if not isinstance(v, DVPSettlement):
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into PrefundedSettlement
        try:
            instance.actual_instance = PrefundedSettlement.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DVPSettlement
        try:
            instance.actual_instance = DVPSettlement.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
