...
class TRLinkJwkPublicKey(BaseModel):
    """
    ...
    use: StrictStr = Field(description="Public key use (encryption)")
    kid: StrictStr = Field(description="Key ID")
    n: StrictStr = Field(description="RSA modulus")
    __properties: ClassVar[List[str]] = ["kty", "e", "use", "kid", "n"]
    ...
    def use_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['enc']):
            raise ValueError("must be one of enum values ('enc')")
        return value
