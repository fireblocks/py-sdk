...
class TransactionRequest(BaseModel):
    ...
    force_sweep: Optional[StrictBool] = Field(default=None, description="For Polkadot, Kusama and Westend transactions only. When set to true, Fireblocks will empty the asset wallet.     **Note:** If set to true when the source account is exactly 1 DOT, the transaction will fail. Any amount more or less than 1 DOT succeeds. This is a Polkadot blockchain limitation.     **Note:** `forceSweep` and `treatAsGrossAmount` can also be used to empty a TON/GRAM wallet.", alias="forceSweep")
    fee_level: Optional[StrictStr] = Field(default=None, description="For UTXO, EVM-based, or Solana blockchains only. Defines the blockchain fee level which will be payed for the transaction. Alternatively, specific fee estimation parameters exist below.", alias="feeLevel")
