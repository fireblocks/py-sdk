...
class ApiClient(object):
    ...
    def __init__(self, configuration: Configuration=None,pool_threads=1):
        # use default configuration if none is provided
        if configuration is None:
            self._configuration = Configuration().get_default()
            self._configuration.options = SDKOptions().get_default()
        else:
            self._configuration = configuration

        if self._configuration.options is None:
            self._configuration.options = SDKOptions().get_default()
        self.pool_threads = pool_threads

        self.rest_client = rest.RESTClientObject(self._configuration.options)
    ...
    def get_user_agent(self):
        ...
        userAgent = f"fireblocks-sdk-py/{version}"
        if not self._configuration.options.anonymousPlatform:
            userAgent += f" ({os.name} {os.uname().release}; {platform.system()} {platform.version()}; {platform.machine()})"
        if self._configuration.options.userAgent:
            userAgent = f"{self._configuration.options.userAgent} {userAgent}"
        return userAgent
    ...
    def __deserialize(self, data, klass):
        ...
        if type(klass) == str:
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(fireblocks_client.models, klass)
    ...
    def get_headers(self,resource_path ,body, header_params):
        token = self._configuration.sign_jwt(resource_path, body)
        return {
            **header_params,
