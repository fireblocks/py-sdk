# coding: utf-8

"""
    Fireblocks API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    The version of the OpenAPI document: 1.6.2
    Contact: support@fireblocks.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""


from __future__ import annotations
from inspect import getfullargspec
import pprint
import re  # noqa: F401
import json


from typing import Optional
from pydantic import BaseModel, Field, StrictStr
from fireblocks_client.models.system_message_info import SystemMessageInfo

class CreateTransactionResponse(BaseModel):
    """
    CreateTransactionResponse
    """
    id: Optional[StrictStr] = Field(None, description="The ID of the transaction.")
    status: Optional[StrictStr] = Field(None, description="The primary status of the transaction. For details, see [Primary transaction statuses.] (https://developers.fireblocks.com/reference/primary-transaction-statuses)")
    system_messages: Optional[SystemMessageInfo] = Field(None, alias="systemMessages")
    __properties = ["id", "status", "systemMessages"]

    class Config:
        populate_by_name = True
        validate_assignment = True
        arbitrary_types_allowed = True

    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.dict(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> CreateTransactionResponse:
        """Create an instance of CreateTransactionResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self):
        """Returns the dictionary representation of the model using alias"""
        _dict = self.dict(by_alias=True,
                          exclude={
                          },
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of system_messages
        if self.system_messages:
            _dict['systemMessages'] = self.system_messages.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> CreateTransactionResponse:
        """Create an instance of CreateTransactionResponse from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return CreateTransactionResponse.parse_obj(obj)

        _obj = CreateTransactionResponse.parse_obj({
            "id": obj.get("id"),
            "status": obj.get("status"),
            "system_messages": SystemMessageInfo.from_dict(obj.get("systemMessages")) if obj.get("systemMessages") is not None else None
        })
        return _obj

