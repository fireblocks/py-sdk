# coding: utf-8

"""
    Fireblocks API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)  # noqa: E501

    The version of the OpenAPI document: 1.6.2
    Contact: support@fireblocks.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""


from __future__ import annotations
from inspect import getfullargspec
import pprint
import re  # noqa: F401
import json



from pydantic import BaseModel
from fireblocks_client.models.network_fee import NetworkFee

class EstimatedNetworkFeeResponse(BaseModel):
    """
    EstimatedNetworkFeeResponse
    """
    low: NetworkFee = ...
    medium: NetworkFee = ...
    high: NetworkFee = ...
    __properties = ["low", "medium", "high"]

    class Config:
        populate_by_name = True
        validate_assignment = True
        arbitrary_types_allowed = True

    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.dict(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> EstimatedNetworkFeeResponse:
        """Create an instance of EstimatedNetworkFeeResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self):
        """Returns the dictionary representation of the model using alias"""
        _dict = self.dict(by_alias=True,
                          exclude={
                          },
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of low
        if self.low:
            _dict['low'] = self.low.to_dict()
        # override the default output from pydantic by calling `to_dict()` of medium
        if self.medium:
            _dict['medium'] = self.medium.to_dict()
        # override the default output from pydantic by calling `to_dict()` of high
        if self.high:
            _dict['high'] = self.high.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> EstimatedNetworkFeeResponse:
        """Create an instance of EstimatedNetworkFeeResponse from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return EstimatedNetworkFeeResponse.parse_obj(obj)

        _obj = EstimatedNetworkFeeResponse.parse_obj({
            "low": NetworkFee.from_dict(obj.get("low")) if obj.get("low") is not None else None,
            "medium": NetworkFee.from_dict(obj.get("medium")) if obj.get("medium") is not None else None,
            "high": NetworkFee.from_dict(obj.get("high")) if obj.get("high") is not None else None
        })
        return _obj

