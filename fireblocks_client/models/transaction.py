...
class Transaction(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["id", "state", "timestamp", "instructionId"]

    @field_validator('state')
    def state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SUBMITTED', 'QUEUED', 'PENDING_AUTHORIZATION', 'PENDING_SIGNATURE', 'BROADCASTING', 'PENDING_3RD_PARTY_MANUAL_APPROVAL', 'PENDING_3RD_PARTY', 'PENDING', 'CONFIRMING', 'CONFIRMED', 'COMPLETED', 'PARTIALLY_COMPLETED', 'PENDING_AML_SCREENING', 'CANCELLING', 'CANCELLED', 'REJECTED', 'BLOCKED', 'FAILED', 'TIMEOUT']):
